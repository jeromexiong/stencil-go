...
((async () => {
  const events = {
    todos: {
      ...
      visitor: function (ns, msg) {
        app.ws = {ns, msg,data: msg.unmarshal()}
        app.visitor = msg.unmarshal()
      }
    }
  };

  const conn = await neffos.dial("ws://localhost:3000/todos/sync", events);
  // const conn = await neffos.dial("wss://localhost:3000/todos/sync", events);
...
var todoStorage = {
  ...
  save: function (todos) {
    ...
    axios.post("/todos", JSON.stringify(todos)).then(response => {
      ...
      ws.emit("Save", JSON.stringify(todos))
      ws.emit("OnChatTest", JSON.stringify(todos))
    });
  }
}
